...
with open(r'E:\testcode\calprofit\asset\src\python\ssq.csv', 'w', encoding='utf-8-sig', newline='') as f:
    csv.writer(f).writerow(['开奖日期', '期号', '中奖号码', '销售额(元)', '中奖注数一等奖', '中奖注数二等奖', '详细'])

for i in range(1,117):  # 目前116页数据
    get_one_page(i).to_csv(r'E:\testcode\calprofit\asset\src\python\ssq.csv', mode='a', encoding='utf_8_sig', header=0, index=0)
    print('第'+str(i)+'页抓取完成')